...
var cntSellect=1;
//---------------------------------------------
//var TotalProblemNum=186;
//honmatsumesuji_info → gensen_info を基準データ
var infoNum=1 //infoの番号：1->厳選
//---------------------------------------------



//var TotalProblemNum=6;
var TotalProblemNum=186;
//var infoNum=2 //infoの番号：2->容易
...
//--------------------------------------------------------------------
for(i=1; i<=TotalProblemNum; i++){
	arr[i]=i;
}
 
//シャッフルアルゴリズム
for(var i=1; i<=TotalProblemNum; i++){
	var j = Math.floor( Math.random() * TotalProblemNum )+1;
	var t = arr[i];
	arr[i] = arr[j];
	arr[j] = t;
}
...
window.onload = function() {
	...
	game.onload = function() {
		...
		//正解数・間違い数 表示
		strSTAR="";
		lblLbl[10].font = "36px serif";
		for(var i=1; i<=problemSTARnum; i++){strSTAR+="★";}
		lblLbl[10].text = " " + strSTAR;
		lblLbl[11].moveTo(250,650);
		lblLbl[11].text = "厳選２５";
		lblLbl[12].text = "問題番号 = " + PrNum;
//		lblLbl[12].text = "正解数 = " + cntCorrect;
//		lblLbl[13].text = "間違い数 = " + cntWrong;
		lblLbl[13].text = "問題数 = " + cntTotalProblem;
		lblLbl[14].text = "正解問題数 = " + cntTotalCorrect;

		for (i=1; i<=20; i++) {
			lblLbl[i].font = "24px 'MSゴシック'";
			lblLbl[i].width= 1000;
			lblLbl[i].x= 400;
			lblLbl[i].y= motigomaTop + 20*i-190;
			lblLbl[i].text = "";
...
console.log("PrCnt = "+PrCnt);
			...
			for(var n=PrCnt; n<=TotalProblemNum; n++){
				//gensen_infoを基準データ
				probleminfo=gensen_info.substr(arr[n],1);
				if(probleminfo==infoNum){
					PrCnt = n; break;
				}
			}

/*
			for(var n=PrCnt+1; n<=TotalProblemNum; n++){
				probleminfo=honmatsumesuji_info.substr(arr[n],1);
				if(probleminfo==infoNum){
					PrCnt = n; break;
				}
			}
...
//			problemSTARnum = star_info.substr(PrNum,1);
...
function ending(){
	lblLbl[5].x = 200; lblLbl[5].y = 650;
	lblLbl[5].font = "48px 'MSゴシック'";
	lblLbl[5].color = "rgb(255,0,0)";
	lblLbl[5].text = "正解！";
//	lblLbl[6].x = 200; lblLbl[6].y = 650;
//	lblLbl[6].font = "148px 'MSゴシック'";
//	lblLbl[6].color = "rgb(255,0,0)";
//	lblLbl[6].text = "正解！";
...
function display() {
	//sprect 表示
//	sprect.moveTo(motigomaLeft + sprectMoti*(banW), motigomaTop );
	for (hm=1; hm<=9; hm++) {
		for (hn=1; hn<=9; hn++) {
			pm = hn-1; pn = 9 - hm;
			i = pm * 9 + pn;
	...
	for (hn=7; hn>=1; hn--) {
		...
		if(motigomanum[hn]==0){
			sptKoma[ii].x = -300;
			labelmotigomanum[hn].x = -300;
		}else{
			sptKoma[ii].x = motigomaLeft + cntMoti * banW;
			//各持駒数表示
			labelmotigomanum[hn].text = motigomanum[hn];
			labelmotigomanum[hn].x = motigomaLeft + cntMoti * banW + banW*3/8;
			cntMoti++;
		}
	}
	//正解数・間違い数 表示
		...
		for(var i=1; i<=problemSTARnum; i++){strSTAR+="★";}
		lblLbl[10].text = " " + strSTAR;
		lblLbl[11].moveTo(200,720);
		lblLbl[11].font = "48px serif";
		lblLbl[11].text = "厳選２５";
		lblLbl[12].text = "問題番号 = " + PrNum;
		lblLbl[12].text = "問題番号 = " + PrNum;
//		lblLbl[12].text = "正解数 = " + cntCorrect;
//		lblLbl[13].text = "間違い数 = " + cntWrong;
		...
		lblLbl[14].text = "正解問題数 = " + cntTotalCorrect;
		lblLbl[5].text = "";
//		lblLbl[9].text = "手数 = " + Ttesuu;
//		lblLbl[8].text = "NowTesuu = " + NowTesuu;
}
var cnt=0;
var YES = 1; var NO = -1;
var FINISH = NO;
		...
		if(MODE12 == first) {
			...
			if(hm1 == 0 && 0 <= hn1 && hn1 <= 7) {
				...
				if(motigomaNum > 0){ //→第２指定に進む
					...
					var TmpMoti=0;
					for (var hn=7; hn>hn1; hn--) {
						if(motigomanum[hn]!=0){
							TmpMoti++;
						}
					}
					sprect.moveTo(motigomaLeft + TmpMoti*(banW), motigomaTop );
					display();
					MODE12 = second; FINISH = YES;
				} else {
					motigomaNum == 0 //→第１指定に戻る
					sprect.moveTo(1000,800);
					display();
					MODE12 = first; FINISH = YES;
				}
...
labelmotigomanum[hn2].text = motigomanum[hn];
					...
					if(NowTesuu >= Ttesuu){
//lblLbl[1].text = "持駒★正解";
lblLbl[15].x = 200; lblLbl[15].y = 650;
lblLbl[15].font = "48px 'MSゴシック'";
lblLbl[15].color = "rgb(255,0,0)";
lblLbl[15].text = "正解！";
						cntTotalCorrect++; ending(); MODE12 = first; FINISH = YES;
					}
